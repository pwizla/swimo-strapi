...
function MonthlyView() {
  ...
  let [selectedMonthNumber, setSelectedMonthNumber] = useState(
    new Date().getMonth() + 1
  );
  ...
  const currentMonthName = `${
    months[selectedMonthNumber - 1].name
  } ${selectedYear}`;
  ...
  return (
    <div className="monthly-view">
      ...
      />
      <Envelopes operations={operations} />
    </div>
  );
}
